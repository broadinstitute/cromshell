...
class TestLogs:
    ...
    def test_print_file_like_value_in_dict(
        self, task_log_metadata, expect_logs, capsys
    ):
    ...
    def test_print_log_file_content(
        self, output_name, output_value, mock_data_path, capsys
    ):
        abs_output_value = Path(mock_data_path).joinpath(output_value)

        with open(abs_output_value, "r") as f:
            file_content = f.read()

        logs_command.print_log_file_content(
            output_name=output_name,
            output_value=str(abs_output_value),
            txt_color=None,
            backend="Local",
        )
        captured = capsys.readouterr()
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                "261ee81a-b6c4-4547-8373-4c879eb24858",
                "Done",
            ),
            (
                {
