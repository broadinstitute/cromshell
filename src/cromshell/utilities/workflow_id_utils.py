...
def resolve_workflow_id(cromshell_input: str or int) -> str:
    if io_utils.is_workflow_id_valid(cromshell_input):
        return cromshell_input
    elif cromshell_input is int:
        return obtain_workflow_id_using_digit(cromshell_input)
    else:
...
def obtain_workflow_id_using_digit(relative_id: int) -> str:
    ...
    with open(cromshellconfig.submission_file_path, "r") as csv_file:
        ...
        if relative_id == 0:
            ...
            row_index = relative_id if relative_id > 0 else total_lines - (
                    abs(relative_id) - 1)
...
def obtain_workflow_id_using_alias(alias_name: str) -> str:
    ...
    with open(cromshellconfig.submission_file_path, "r") as csv_file:
        ...
        LOGGER.error(
            "Unable to find alias '%s' in submission file '%s'", alias_name,
            cromshellconfig.submission_file_path
        )
        raise ValueError(
            f"Unable to find alias {alias_name} in submission file")
